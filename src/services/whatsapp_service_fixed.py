...
class WhatsAppService:
    ...
    def __init__(self):
        ...
        self.min_message_interval: float = 1.0        # Concurrency limit
        self._semaphore = asyncio.Semaphore(Config.MAX_REQUESTS_PER_MINUTE)
        # Reusable HTTP session (lazy initialization to avoid event loop issues)
        self.session = None

    async def _ensure_session(self) -> aiohttp.ClientSession:
        """Ensure HTTP session is initialized (lazy loading)"""
